...
module.exports = function elasticSearchPlugin(schema, options) {
  var mapping = getMapping(schema),
    ...
    //esClient = new elasticsearch.Client(host, options),
...
function createMappingIfNotPresent(client, indexName, typeName, schema, cb) {
  generator.generateMapping(schema, function(err, mapping) {
    var completeMapping = {};
    var localhost = 'http://localhost:9200/';
    ...
    // console.log(completeMapping[typeName]);
    // createIndex(indexName, typeName, completeMapping, function(err) {
    //   if (err) {
    //     removeIndex(indexName, cb);
    //     createIndex(indexName, typeName, completeMapping, cb);
    //   }
    // });
  });
}
...
function removeIndex(index, callback) {
  var localhost = 'http://localhost:9200/';
  request.del(localhost + index + '/', function(err) {
    if (err) return callback(err);
    callback();
  });
}

function createIndex(index, type, mapping, callback) {
  var localhost = 'http://localhost:9200/';
  request.put({
    url: localhost + index + '/',
    body: JSON.stringify({
